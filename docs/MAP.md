...
- `Sources/MDNotesApp/MainMenu.swift` — code-built menu bar (P-2); targetless items resolved through the responder chain. `MainMenu` (App/File/Edit/Note/View/Window menus; File holds the `New from Template` placeholder submenu (TP-6, filled in M9.5) and Close; View holds Bigger/Smaller/Actual Size (E-8) and the backlinks toggle; Window is the standard Minimize/Zoom/Bring All to Front).
- `Sources/MDNotesApp/MainWindowController.swift` — the single main window (W-1), floating and following the active Space (W-5): search field, list, editor wiring; commit/rename/delete/open-link/insert-image actions; a plain click in the editor opens a thumbnail's image (E-9) or searches a tag (T-4); View menu font size actions (E-8); one `ThumbnailCache` shared by list and editor (PF-8). `MainWindowController.attach`, `commitQuery`, `commitTitle`, `openLink`, `clickInEditor`, `openThumbnail`, `search`, `makeTextBigger/Smaller/ActualSize`, `validateMenuItem`, `windowLevel`, `overlayLevel`.
...
- `Sources/MDNotesApp/NoteListController.swift` — table data source/delegate over one `SearchIndex.Results`; inline title editing; date refresh on `NSCalendarDayChanged` and key window (S-6, S-9); row thumbnails looked up in `thumbnails` under `imageRoot` as rows are made, requested when not cached (S-11, PF-8), and asked for again when the image file changes on disk (X-1). `NoteListController.show`, `select`, `beginEditingTitle`, `dateText`, `refreshDates`, `now`, `thumbnails`, `imageRoot`, `thumbnailPixelSize`, `thumbnailURL(for:)`, `imagesDidChange`.
...
- `Sources/MDNotesApp/EditorTextView.swift` — intercepts Cmd-click and Cmd-Return for link opening (K-3); copy, cut and drag write the file's text when attachments are on show (E-9). `EditorTextView`, `characterIndex`, `writeSelection(to:types:)`.
- `Sources/MDNotesApp/EditorStyler.swift` — syntax styling of headings, links, tags, code (code in the monospaced font, E-8); paragraph-scoped restyle (E-2, E-3); scans `EditorText` and maps token ranges back to the storage (E-9). `EditorStyler.restyleAll`, `restyleAfterEdit`, `restyleLinks`, `TokenStyle`, `baseFont`, `codeFont`.
...
- `Sources/MDNotesTestSupport/SyntheticLibrary.swift` — deterministic fake library on disk: nesting, tags, wikilinks, large notes, `imageFraction` (default 10 %, PF-8) of notes embedding a generated PNG of their own under `i/`, and `largeNoteEmbeds` (default 0) PNGs embedded through each large note (E-9); a hand-written PNG encoder needing only Foundation. `SyntheticLibrary.generate`, `Options`, `pngData(seed:width:height:)`, `imageName(forNoteAt:)`, `imageName(forNoteAt:embed:)`, `SplitMix64`.
...
- `Tests/MDNotesAppTests/MenuSmokeTests.swift` — the whole menu bar's titles and key equivalents as one table (P-2), the File menu's template placeholder (TP-6); items found by action, sent down the responder chain; Cmd-W and close button hide, Dock reopen, quit writes edits (W-4).
...
- `Tests/MDNotesAppTests/EditorPerfTests.swift` — keystroke, paragraph restyle, thumbnail reconciliation, redraw in a 1 MB note with 50 embeds' thumbnails on show (PF-3, E-3, E-9).
...
- Templates: `Sources/MDNotesCore/TemplateStore.swift` lists `templates/` and reads one file (TP-1, TP-2); `Core/TemplateParser.swift` parses and expands it (TP-2, TP-3); `LibraryController.templateNames` is the live list (TP-7).
