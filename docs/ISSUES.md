...
- [ ] I-11 bug `LaunchPerfTests.testPF1 cold launch` (2026-09-15): the first (cold) launch in the process measured 444 to 600 ms against the 300 ms budget in 20+ attempts on 2026-09-15 with the unchanged M10.2 tree (the median of the seven launches stayed at 200 to 220 ms; a run at 09:10 the same day passed). The human's direction: do not retry the gate or make it load-aware; profile the cold path (Instruments or sample on the release test binary, compare against the last green run, look at what M8 to M10 added to launch: ThumbnailCache, TemplateStore listing, the wider MarkdownScanner in the styler and BodySnippet, EditorThumbnails reconciliation) and make cold launch faster until PF-1 passes with margin. See Q4.
